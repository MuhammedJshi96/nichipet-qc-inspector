...
def inject_css():
    st.markdown(
        ...
        .compact-grid {
            display:grid;
            grid-template-columns: 90px 1.8fr 1.2fr 90px 90px 120px 160px 180px;
            gap:8px;
            align-items:center;
        }
        ...
        .compact-row {
            background:#fff;
            border:1px solid rgba(15,79,56,0.08);
...
def normalize_photo(photo):
    ...
    if isinstance(photo, dict):
        return {
            "file_name": photo.get("file_name", "image"),
            "mime_type": photo.get("mime_type", "image/jpeg"),
            "image_blob": photo.get("image_blob"),
        }
    return {
        "file_name": getattr(photo, "file_name", "image"),
        "mime_type": getattr(photo, "mime_type", "image/jpeg"),
        "image_blob": getattr(photo, "image_blob", None),
    }
...
def build_history_result(record):
    ...
    elif result.metadata.mode == "routine":
        result.final_use_decision = "ROUTINE REVIEW ONLY / NOT OFFICIAL"
...
def decision_pill_html(result):
    ...
    if result.metadata.mode == "official" and result.overall_status == "PASS":
        return "<span class='status-pill pill-pass'>PASS</span>"
    if result.metadata.mode == "official":
        return "<span class='status-pill pill-fail'>NEEDS REVIEW</span>"
    return "<span class='status-pill pill-neutral'>NON-OFFICIAL</span>"


def history_status_pill(row):
    demo_detected = (
    ...
    if demo_detected:
        return "<span class='status-pill pill-demo'>DEMO</span>"
    if row.mode == "official" and row.overall_status == "PASS":
        return "<span class='status-pill pill-pass'>PASS</span>"
    if row.mode == "official":
        return "<span class='status-pill pill-fail'>REVIEW</span>"
    return "<span class='status-pill pill-neutral'>NON-OFFICIAL</span>"
...
def render_results_block(result, repo=None, allow_save=True):
    ...
    st.markdown(
        ...
        <div class="metric-grid">
            <div class="metric-card"><div class="metric-label">Inspection ID</div><div class="metric-value">{result.metadata.inspection_id}</div></div>
            <div class="metric-card"><div class="metric-label">Model</div><div class="metric-value">{result.metadata.model_code}</div></div>
            <div class="metric-card"><div class="metric-label">Pipette number</div><div class="metric-value">{result.metadata.pipette_serial_number}</div></div>
            <div class="metric-card"><div class="metric-label">Decision</div><div class="metric-value">{decision_pill_html(result)}</div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if result.low_volume_note:
        st.warning(result.low_volume_note)
    if result.non_compliant_conditions:
        st.warning("Checklist has non-compliant conditions.")

    for idx, p in enumerate(result.point_results, start=1):
        st.markdown(f"<div class='point-title'>Point {idx} — Vs = {p.selected_volume_ul} μL</div>", unsafe_allow_html=True)

        std_dev_ul = None
        if getattr(p, "corrected_volumes_ul", None) and len(p.corrected_volumes_ul) > 1:
            std_dev_ul = pd.Series(p.corrected_volumes_ul, dtype="float64").std(ddof=1)

        cols = st.columns(7)
        cols[0].metric("Mean", f"{p.mean_volume_ul:.4f} μL")
        cols[1].metric("Std Dev", "N/A" if std_dev_ul is None else f"{std_dev_ul:.4f} μL")
        cols[2].metric("es %", f"{p.systematic_error_percent:.4f}%")
        cols[3].metric("|es| %", f"{p.absolute_systematic_error_percent:.4f}%")
        cols[4].metric("CV %", "N/A" if p.cv_percent is None else f"{p.cv_percent:.4f}%")
        cols[5].metric("AC limit", f"{p.ac_limit_percent:.4f}%")
        cols[6].metric("CV limit", f"{p.cv_limit_percent:.4f}%")

        point_pill = "<span class='status-pill pill-pass'>PASS</span>" if p.passed else "<span class='status-pill pill-fail'>FAIL</span>"
        ...
        if photo_items:
            ...
            for i, (rep_no, photo_raw) in enumerate(photo_items):
                ...
                with photo_cols[i % 4]:
                    st.markdown(f"<div class='thumb-card'><strong>Replicate {rep_no}</strong></div>", unsafe_allow_html=True)
                    if photo and photo["image_blob"] is not None:
                        st.image(photo["image_blob"], caption=photo["file_name"], use_container_width=True)
    ...
    if allow_save and repo is not None:
        ...
        saved_key = f"saved_once_{result.metadata.inspection_id}"
        just_saved_key = f"just_saved_{result.metadata.inspection_id}"
        ...
        if already_saved:
            ...
            if button_slot.button(t("save"), use_container_width=True):
                try:
                    ...
                    st.session_state["selected_page"] = t("results")
                    st.session_state["nav_override"] = t("results")
        ...
        c2.download_button("Download HTML", data=html_path.read_text(encoding="utf-8"), file_name=html_path.name, mime="text/html", use_container_width=True)
        c3.download_button("Download PDF", data=pdf_path.read_bytes(), file_name=pdf_path.name, mime="application/pdf", use_container_width=True)
        c4.download_button("Download CSV", data=csv_path.read_text(encoding="utf-8"), file_name=csv_path.name, mime="text/csv", use_container_width=True)
    else:
        c1, c2, c3 = st.columns(3)
        c1.download_button("Download HTML", data=html_path.read_text(encoding="utf-8"), file_name=html_path.name, mime="text/html", use_container_width=True)
        c2.download_button("Download PDF", data=pdf_path.read_bytes(), file_name=pdf_path.name, mime="application/pdf", use_container_width=True)
        c3.download_button("Download CSV", data=csv_path.read_text(encoding="utf-8"), file_name=csv_path.name, mime="text/csv", use_container_width=True)


def render_settings(repo):
    st.subheader(t("settings"))
    st.session_state["lang"] = st.selectbox(t("language"), ["en", "ja"], index=["en", "ja"].index(st.session_state.get("lang", "en")))
    st.markdown("### " + t("operator"))
    c1, c2 = st.columns([2, 1])
    new_op = c1.text_input("New operator name")
    if c2.button(t("add_operator")) and new_op.strip():
        repo.add_operator(new_op.strip())
        set_page(t("settings"))
    ...
    if operators:
        ...
        selected = st.selectbox("Saved operators", list(labels.keys()))
...
def render_mode_page(repo, mode_key, draft_key, official_mode=False, routine_mode=False):
    ...
    if official_mode:
        st.markdown('<div class="small-banner">Official mode uses exactly 10 replicates per point</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="small-banner">Non-official mode: replicate count is configurable</div>', unsafe_allow_html=True)
...
def render_results(repo):
    ...
    render_results_block(get_last_result(), repo=repo, allow_save=True)
...
def render_dashboard(repo):
    ...
    st.markdown("<div class='compact-head compact-grid'><div>Pipette</div><div>Inspection ID</div><div>Operator</div><div>Mode</div><div>Status</div><div>Created</div><div></div><div></div></div>", unsafe_allow_html=True)
    ...
    for pipette in PIPETTE_NUMBERS:
        ...
        if row is None:
            ...
            st.markdown(
                f"<div class='compact-row compact-grid'><div><strong>{pipette}</strong></div><div>{row.inspection_id}</div><div>{row.operator_name or '-'}</div><div>{row.mode}</div><div>{history_status_pill(row)}</div><div>{row.created_at.strftime('%Y-%m-%d %H:%M:%S')}</div><div></div><div></div></div>",
                unsafe_allow_html=True,
            )
...
def render_history(repo):
    ...
    for row in rows:
        ...
        cols[5].markdown(history_status_pill(row), unsafe_allow_html=True)
...
def run_app():
    ...
    with session_factory() as session:
        ...
        if "current_page" not in st.session_state:
            st.session_state["current_page"] = t("dashboard")
